...
public class ArmAndIntakeFunctions {
        ...
        public void collectSample() {
            ...
            if (isArmAtTarget(0)) {
                // Indicate that the collection process is ready
                telemetry.addData("Status", "Ready to collect items. Press the right trigger to start.");
                telemetry.update();

                // Check if the 'a' button is pressed on gamepad2
                if (gamepad2.a) {
                    intake.setIntakePower(1.0); // Start intake for collection
                    telemetry.addData("Status", "Collecting samples...");
                    telemetry.update();
